...
<body>
    ...
    <!-- Add this new button for managing addons -->
...
<div class="modal fade" id="manageAddonsModal" tabindex="-1">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form id="quickAddonForm">
                            <div class="row">
                                <div class="col-md-5">
                                    <input type="text" class="form-control" id="newAddonName" placeholder="Addon name" required>
                                </div>
                                <div class="col-md-3">
                                    <div class="input-group">
                                        <span class="input-group-text">₱</span>
                                        <input type="number" class="form-control" id="newAddonPrice" placeholder="Price" step="0.01" min="0" required>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <input type="number" class="form-control" id="newAddonMaxQty" placeholder="Max" value="1" min="1">
                                </div>
                                <div class="col-md-2">
                            ...
                            <div class="row mt-2">
                                <div class="col-md-12">
                                    <textarea class="form-control" id="newAddonDescription" placeholder="Description (optional)" rows="2"></textarea>
                                    <div class="form-check form-switch mt-2">
                                        <input class="form-check-input" type="checkbox" id="newAddonIsGlobal">
                                        <label class="form-check-label" for="newAddonIsGlobal">
                                            Make this addon global (available for all products)
                                        </label>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        function submitStockUpdate() {
            ...
            $.ajax({
                url: '../api/products.php?action=update_stock',
                type: 'POST',
...
let selectedAddons = new Set(); // Store selected addon IDs
...
function manageAddons(productId, productName) {
    ...
    loadGlobalAddons();
    loadAvailableAddons();
    loadProductSpecificAddons(productId);
    
    document.getElementById('quickAddonForm').reset();
    
    new bootstrap.Modal(document.getElementById('manageAddonsModal')).show();
}

function loadProductSpecificAddons(productId) {
    fetch(`../api/get-product-addons.php?product_id=${productId}`)
        .then(response => response.json())
        .then(data => {
            const container = document.getElementById('productAddonsList');
            
            if (data.success && data.addons.length > 0) {
                let html = '';
                selectedAddons.clear(); // Clear first
                                    ...
                                    <div class="d-flex justify-content-between align-items-start">
                                        <div>
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <span class="badge bg-secondary">Max: ${addon.max_quantity}</span>
                                            </div>
                                        </div>
                                        <div>
                ...
                loadAvailableAddons(); // Refresh dropdown
...
function loadGlobalAddons() {
    fetch('../api/get-addons.php?type=global')
        .then(response => response.json())
        .then(data => {
            const container = document.getElementById('globalAddonsList');
        ...
        .catch(error => {
            console.error('Error loading global addons:', error);
            document.getElementById('globalAddonsList').innerHTML = `
                <div class="text-center py-3">
...
function loadAvailableAddons() {
    fetch('../api/get-addons.php?type=all')
        .then(response => response.json())
        .then(data => {
            const select = document.getElementById('availableAddons');
            
            if (data.success && data.addons.length > 0) {
                let options = '<option value="">Select addon to assign...</option>';
                data.addons.forEach(addon => {
                    if (!selectedAddons.has(addon.id)) {
                        options += `<option value="${addon.id}">${escapeHtml(addon.name)} (+₱${addon.price})</option>`;
                    }
                });
                select.innerHTML = options;
            } else {
                select.innerHTML = '<option value="">No addons available</option>';
            }
        })
        .catch(error => {
            console.error('Error loading addons:', error);
            document.getElementById('availableAddons').innerHTML = '<option value="">Error loading addons</option>';
        });
}

function loadProductAddons(productId) {
    fetch(`../api/get-addons.php?product_id=${productId}`)
        ...
        .then(data => {
            ...
            if (data.success && data.addons.length > 0) {
                ...
                // Clear selectedAddons first
                selectedAddons.clear();
                
                data.addons.forEach(addon => {
                    // Only show and track NON-GLOBAL addons
                    if (!addon.is_global || addon.is_global == 0) {
                        selectedAddons.add(addon.id);
                        
                        html += `
                            <div class="col-md-6 mb-3">
                                <div class="card addon-item-card product-addon" id="addon_card_${addon.id}">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-start">
                                            <div>
                                                <h6 class="mb-1">${escapeHtml(addon.name)}</h6>
                ...
                // If we have non-global addons to show
                if (html) {
                    container.innerHTML = html;
                } else {
                    showEmptyAddonsState();
                }
...
function showEmptyAddonsState() {
    const container = document.getElementById('productAddonsList');
    container.innerHTML = `
        <div class="text-center py-5">
            <i class="fas fa-plus-circle fa-2x text-muted mb-2"></i>
            <p class="text-muted">No product-specific addons assigned</p>
            <small>Use the dropdown above to assign addons</small>
        </div>
    `;
}
...
function assignAddonToProduct() {
    ...
    // Reload product addons
    loadProductAddons(currentAddonProductId);
}

function removeProductAddon(addonId) {
    if (!confirm('Remove this addon from product?')) return;
    
    const card = document.getElementById(`addon_card_${addonId}`);
    ...
    // Update database
    fetch('../api/update-products-addons.php', {
        method: 'POST',
    ...
    .then(data => {
        if (data.success) {
            showAlert('success', 'Addon removed');
            if (card) {
                card.remove();
                if (selectedAddons.size === 0) {
                    document.getElementById('productAddonsList').innerHTML = `
                        <div class="text-center py-5">
                            <i class="fas fa-plus-circle fa-2x text-muted mb-2"></i>
                            <p class="text-muted">No product-specific addons</p>
                        </div>
...
function saveProductAddons() {
    const productId = currentAddonProductId;
    ...
    .then(data => {
        if (data.success) {
            ...
            // Update the has_addons flag on the product
            updateProductAddonsFlag(productId, addonsArray.length > 0);
            
            // Close modal after a delay
            setTimeout(() => {
                const modal = bootstrap.Modal.getInstance(document.getElementById('manageAddonsModal'));
                modal.hide();
            }, 1500);
        } else {
            showAlert('error', data.message);
...
function updateProductAddonsFlag(productId, hasAddons) {
    fetch('../api/products.php?action=update_addons_flag', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    .then(data => {
        if (!data.success) {
            console.error('Failed to update addons flag:', data.message);
        }
    })
    .catch(error => {
        console.error('Error updating addons flag:', error);
    });
}

// Quick addon creation
document.getElementById('quickAddonForm').addEventListener('submit', function(e) {
    e.preventDefault();
    
    const name = document.getElementById('newAddonName').value.trim();
    const price = document.getElementById('newAddonPrice').value;
    const maxQty = document.getElementById('newAddonMaxQty').value || 1;
    const description = document.getElementById('newAddonDescription').value.trim();
    const isGlobal = document.getElementById('newAddonIsGlobal').checked;
    
    if (!name || !price) {
        showAlert('warning', 'Please provide addon name and price');
        return;
    }
    ...
    fetch('../api/addons.php?action=create', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    .then(data => {
        if (data.success) {
            ...
            // Reset form
            document.getElementById('quickAddonForm').reset();
            document.getElementById('newAddonIsGlobal').checked = false;
            
            // Reload addon lists
            loadGlobalAddons();
            loadAvailableAddons();
            
            // If not global and we're managing a product, auto-assign it
            if (!isGlobal && currentAddonProductId) {
                setTimeout(() => {
                    selectedAddons.add(data.addon_id);
                    loadProductAddons(currentAddonProductId);
                }, 500);
            }
        } else {
